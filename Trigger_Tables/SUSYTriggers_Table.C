...
void SUSYTriggers_Table() {
    ...
    //--- get yields
    vector<vector<vector<int>>> yields; // [trigger][file][flavor]
    ...
    for (string trigger : triggers) {
        ...
        vector<vector<int>> trigger_yields, trigger_baseline_yields;
        for (string file_name : file_names) {
            ...
            vector<int> file_yields, file_baseline_yields;
            for (string flavor : flavors) {
                file_yields.push_back(tch->GetEntries((trigger + " && " + flavor).c_str()));
                file_baseline_yields.push_back(tch->GetEntries((baseline + " && " + trigger + " && " + flavor).c_str()));
            }
            trigger_yields.push_back(file_yields);
            trigger_baseline_yields.push_back(file_baseline_yields);
            delete tch;
        }
        yields.push_back(trigger_yields);
        baseline_yields.push_back(trigger_baseline_yields);
    }
    ...
    for (int j=0; j<triggers.size(); j++) {
        ...
        for (int i=0; i<file_names.size(); i++)
            fout << file_labels[i] << " & " << yields[j][i][0] << " & " << yields[j][i][1] << " & " << yields[j][i][2] << " \\\\" << endl;
    ...
    for (int j=0; j<triggers.size(); j++) {
        ...
        fout << "\\hline" << endl;
        for (int i=0; i<file_names.size(); i++)
            fout << file_labels[i] << " & " << (float)baseline_yields[j][i][0]/baseline_yields[0][i][0]*100 << " & " << (float)baseline_yields[j][i][1]/baseline_yields[0][i][1]*100 << " & " << (float)baseline_yields[0][i][2]/baseline_yields[0][i][2]*100 << " \\\\" << endl;
        fout << "\\end{tabular}" << endl;
        fout << "\\end{center}" << endl;
        fout << "\\end{table}" << endl;
        fout << endl;
